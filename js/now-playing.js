		...
		if(!library_data) {
			...
			if(old_result != current_result) {
				...
				$(".playing-info").one("animationend webkitAnimationEnd oAnimationEnd MSAnimationEnd", function(){
					$(".playing-info .title").html('<a title="' + row.TITLE + '" href="' + row.TRACK_PERMALINK + '">' + row.TITLE + '</a>');
					$(".playing-info .artist").html('<a title="' + row.ARTIST + '" href="' + row.ARTIST_PERMALINK + '">' + row.ARTIST + '</a>');
